...
# 13 "MCAL_layer/USART/../GPIO/hal_gpio.h"
...
# 12 "MCAL_layer/USART/../GPIO/../std_libraries.h"
...
# 14 "MCAL_layer/USART/../GPIO/../std_libraries.h" 2
...
# 12 "MCAL_layer/USART/../GPIO/../compiler.h" 2
...
# 14 "MCAL_layer/USART/../Interrupt/mcal_internal_interrupt.h"
...
# 60 "MCAL_layer/USART/../Interrupt/mcal_interrupt_config.h"
typedef enum{
    INTERRUPT_LOW_PRIORITY = 0 ,
    INTERRUPT_HIGH_PRIORITY,

}interrupt_priority_cfg;
# 15 "MCAL_layer/USART/../Interrupt/mcal_internal_interrupt.h" 2
...
# 2 "MCAL_layer/USART/hal_usart.c" 2

static __attribute__((inline)) Std_ReturnType EUSART_ModeSelection(const usart_t *_usart_obj);
...
static void(*EUSART_OverrunHandler)(void) = ((void*)0);
...
Std_ReturnType EUSART_ReadByteBlocking(const usart_t *_usart_obj , uint8 *_data){
    Std_ReturnType ret = (Std_ReturnType)0x00;
    if (((void*)0) == _usart_obj && ((void*)0) == _data){
        ret = (Std_ReturnType)0x00;
    }
    else{
        while(!(PIR1bits.RCIF));
...
static __attribute__((inline)) Std_ReturnType EUSART_ModeSelection(const usart_t *_usart_obj){
    ...
    if (((void*)0) == _usart_obj){
        ...
        if(1 == _usart_obj->usart_mode_selection){
            (TXSTAbits.SYNC = 1);
            switch (_usart_obj->baudrate_gen_cfg){
                case BAUDRATE_SYNC_8BIT:
                    (BAUDCONbits.BRG16 = 0);
                    baud_rate_temp = ((8000000UL / (float)(_usart_obj->baudrate) ) / 4) - 1;
                    break;
        ...
        else if(0 == _usart_obj->usart_mode_selection){
            (TXSTAbits.SYNC = 0);
            switch (_usart_obj->baudrate_gen_cfg){
                case BAUDRATE_ASYNC_8BIT_LOW_SPEED :
                    (BAUDCONbits.BRG16 = 0);
                    (TXSTAbits.BRGH = 0);
                    baud_rate_temp = ((8000000UL / (float)(_usart_obj->baudrate) ) / 64) - 1;
                    break;

                case BAUDRATE_ASYNC_8BIT_HIGH_SPEED:
                    (BAUDCONbits.BRG16 = 0);
                    (TXSTAbits.BRGH = 1);
                    baud_rate_temp = ((8000000UL / (float)(_usart_obj->baudrate) ) / 16) - 1;
                    break;
...
static void EUSART_TX_Init(const usart_t *_usart_obj){
    if (1 == _usart_obj->usart_tx_cfg.usart_tx_enable ){
        (TXSTAbits.TXEN = 1);
        if (_usart_obj->usart_tx_cfg.usart_tx_interrupt_enable == 1){
            (PIE1bits.TXIE = 1);

                    (PIE1bits.TXIE = 1);
                    EUSART_TxInterruptHandler = (_usart_obj->EUSART_TxInterruptHandler);
# 188 "MCAL_layer/USART/hal_usart.c"
                    (INTCONbits.GIE=1);
                    (INTCONbits.PEIE=1);
        ...
        else if(_usart_obj->usart_tx_cfg.usart_tx_interrupt_enable == 0){
            (PIE1bits.TXIE = 0);
        }
        else{ }

        if(_usart_obj->usart_tx_cfg.usart_tx_9bit_enable == 1 ){
            (TXSTAbits.TX9 = 1);
        }
        else if (_usart_obj->usart_tx_cfg.usart_tx_9bit_enable == 0 ){
            (TXSTAbits.TX9 = 0);
        }
        else{ }
    ...
    else if(0 == _usart_obj->usart_tx_cfg.usart_tx_enable){
        (TXSTAbits.TXEN = 0);
    }
}
...
static void EUSART_RX_Init(const usart_t *_usart_obj){
    if (1 == _usart_obj->usart_rx_cfg.usart_rx_enable ){
        (RCSTAbits.CREN = 1);
        if (_usart_obj->usart_rx_cfg.usart_rx_interrupt_enable == 1){
            (PIE1bits.RCIE = 1);
...
# 238 "MCAL_layer/USART/hal_usart.c"
        ...
        else if(_usart_obj->usart_rx_cfg.usart_rx_interrupt_enable == 0){
            (PIE1bits.RCIE = 0);
        }
        else{ }

        if(_usart_obj->usart_rx_cfg.usart_rx_9bit_enable == 1 ){
            (RCSTAbits.RX9 = 1);
        }
        else if (_usart_obj->usart_rx_cfg.usart_rx_9bit_enable == 0 ){
            (RCSTAbits.RX9 = 0);
        }
        else{ }

    }
    else if(0 == _usart_obj->usart_rx_cfg.usart_rx_enable){
        (RCSTAbits.CREN = 0);
    }
}
